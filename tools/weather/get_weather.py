...
class GetWeather(object):
    ...
    def __init__(self, city: str, token_or_path: str):
        ...
        self.info_dict = {
            ...
            "city": self.loacation,
            "forcastype": "daily",
            "ASKPARAM": {"dailysteps": self.dailysteps, "alert": "true"},
        }
    ...
    def get_weather(self):
        ...
        """
        try:
            r = requests.get(self.askurl, params=self.info_dict["ASKPARAM"], timeout=5)
    ...
    def pross_weather(self):
        ...
        try:
            ...
            dateDraft = date + "。"
            temperatureDraft = (
                "温度：" + f"{temperature['max']} ~ {temperature['min']}" + "℃。 "
            )
            windspeedDraft = (
                "风力：" + f"{wind['min']['speed']} ~ {wind['max']['speed']}" + "m/s。 "
            )
            winddrectionDraft = "风向：" + Data.get_wind_direction(
                wind["avg"]["direction"]
            )
            precipitationDraft = "降水可能性：" + f"{precipitation['probability']}" + "%。 "
            skyconDraft = "天气" + Data.WEATHER_DICT[skycon["value"]] + "。"
            pm25Draft = "明天PM2.5: " + f"{air_quality['pm25'][1]['avg']}" + "μg/m³。 "
